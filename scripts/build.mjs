...
set /a WAIT_COUNT+=1
>nul 2>&1 powershell -Command "try { $r = Invoke-WebRequest -Uri 'http://127.0.0.1:17870/api/health' -TimeoutSec 2; if ($r.StatusCode -eq 200) { exit 0 } else { exit 1 } } catch { exit 1 }" && (
    echo [launcher] 后端已就绪
    goto :launch
)
